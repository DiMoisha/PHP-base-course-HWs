<?php
    ...
    function _checkout() {
        ...
        $meta_descr     = "Оформление заказа. Корзина покупок. ООО ЛАГОС";
        $meta_keywords  = "корзина покупок, корзина, дорожные материалы, асфальтобетонный завод, ООО ЛАГОС";
        $menu_item      = "checkout";
        return [GetCheckout(), $pageTitle, $pageHeading, $meta_descr, $meta_keywords, $menu_item];
    }
    ...
    function _completed() {
        ...
        if (isset($_GET['orderid'])) {
            $orderId = intval(filter_var(trim($_GET['orderid']), FILTER_SANITIZE_STRING));
        }

        $pageTitle      = "Заказ отправлен в обработку";
        $pageHeading    = "Заказ отправлен в обработку";
        $meta_descr     = "Заказ отправлен в обработку. Корзина покупок. ООО ЛАГОС";
        $meta_keywords  = "корзина покупок, корзина, дорожные материалы, асфальтобетонный завод, ООО ЛАГОС";
        $menu_item      = "completed";
        return [Completed($orderId), $pageTitle, $pageHeading, $meta_descr, $meta_keywords, $menu_item];
    }

    function _orders() {
        require_once LOCAL_PATH_ROOT."/Domain/Controllers/OrderController.php";
        $pageTitle      = "Журнал заказов";
        $pageHeading    = "Журнал заказов";
        $meta_descr     = "Журнал заказов. ООО ЛАГОС";
        $meta_keywords  = "Журнал заказов, асфальт, бетон, щебень, битум, асфальтобетонный завод, АБЗ-4";
        $menu_item      = "orders";
        return [Index(), $pageTitle, $pageHeading, $meta_descr, $meta_keywords, $menu_item];
    }
    ...
    function _cancelorder() {
        require_once LOCAL_PATH_ROOT."/Domain/Controllers/OrderController.php";
        if (isset($_GET['orderid'])) {
            return CancelOrder(intval(filter_var(trim($_GET['orderid']), FILTER_SANITIZE_STRING)));
        }
    ...
    function _setorder() {
        ...
        if (isset($_GET['orderid'])) {
            ...
            if(isset($_GET['status'])) {
                return OrderStatus($orderId, intval(filter_var(trim($_GET['status']), FILTER_SANITIZE_STRING)));
            }

            if(isset($_GET['deliverysum'])) {
                return OrderDeliverySum($orderId, floatval(filter_var(trim($_GET['deliverysum']), FILTER_SANITIZE_STRING)));
            }
        }
